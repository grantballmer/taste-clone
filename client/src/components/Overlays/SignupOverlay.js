...
const SignupOverlay = props => {
    const classVar = props.info.display ? 'overlay-show' : '';
    return (
        <div className={`overlay overlay-signup ${classVar}`}>
            <p className="overlay__cancel" onClick={props.remove}>{"\u2716"}</p>
            <div className="popup">
                ...
                <Link to="/login">Already have a profile? <span>Log in >></span></Link>
            </div>
        </div>
    );
};
