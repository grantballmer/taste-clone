...
import RateMovie from "../components/Overlays/RateMovie";
...
class GridParent extends React.Component {
  ...
  updateOverlayInfo = (movie) => {
    ...
    console.log(movie);
    const index = this.props.seen.findIndex(movie.id);
    ...
    return (
      <div className="main-padding">
        ...
        {this.state.displayOverlay && */}
          {auth ? (
            <RateMovie remove={this.removeOverlay} info={this.state} />
          ) : (
        ...
        <MovieGrid page={page} url={APICalls[funcName](page, optionalArg)} updateOverlayInfo={this.updateOverlayInfo} />
      </div>
    );
  }  
};
